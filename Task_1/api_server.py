...
def initialize_db(DB_path=None, seed_data=True):
    if DB_path is None:
...
def create_app(db_path=None, seed_data=True):
    resolved_DB_path = Path(db_path) if db_path is not None else DEFAULT_DB_PATH
    initialize_db(resolved_DB_path, seed_data=seed_data)
    ...
    system = TicketSystem(DB_path=resolved_DB_path)
    ...
    def require_user(payload):
        ...
        if user_id is None:
            return None
        return system.auth_sys.get_user_by_id(user_id)
    ...
    def process_expiries():
        system.booking_sys.process_expired_bookings()
    ...
    def api_auth_login():
        ...
        user, message = system.auth_sys.login(
    ...
    def api_auth_register():
        payload = request.get_json(silent=True) or {}
        user, message = system.auth_sys.register(
            payload.get("username", ""),
            payload.get("pwd", ""),
    ...
    def api_events_list():
        ...
        if user is None:
            return make_json(False, "User not found", error_code="USER_NOT_FOUND"), 401
        process_expiries()
        events = [serialize_event(event) for event in system.event_sys.list_events()]
    ...
    def api_events_detail():
        ...
        event = system.event_sys.get_event(event_id)
        if event is None:
            return make_json(False, "Event not found", error_code="EVENT_NOT_FOUND"), 404

        ticket_types = [
            serialize_ticket_type(ticket_type)
            for ticket_type in system.event_sys.get_ticket_types(event_id)
        ]
        return make_json(
            True,
            "Event detail loaded",
    ...
    def api_bookings_create():
        ...
        booking, message = system.booking_sys.create_booking(
            user.user_id,
            event_id,
            ticket_type_id,
            quantity,
    ...
    def api_bookings_pay():
        ...
        success, message = system.booking_sys.pay_booking(
            booking_id,
            user.user_id,
            payment_pin,
        )
        ...
        fresh_user = system.auth_sys.get_user_by_id(user.user_id)
        return make_json(
            True,
            message,
            data={
    ...
    def api_balance_get():
        ...
        balance = system.balance_sys.get_balance(user.user_id)
    ...
    def api_balance_add():
        ...
        payment_pin = str(payload.get("payment_pin", "")).strip()
        pin_ok, pin_message = system.auth_sys.verify_payment_pin(
            user.user_id,
            payment_pin,
        )
        if not pin_ok:
        ...
        new_balance = system.balance_sys.add_balance(user.user_id, amount)
    ...
    def api_bookings_my():
        ...
        now = now_text()
        rows = []
        for row in system.booking_sys.get_user_bookings(user.user_id):
            row_data = dict(row)
    ...
    def api_admin_events_add():
        ...
        event_id = system.event_sys.add_event(title, description, location, event_date)
    ...
    def api_admin_ticket_types_add():
        ...
        system.event_sys.add_ticket_type(event_id, name, price, stock)
    ...
    def api_admin_bookings_status():
        ...
        rows = [dict(row) for row in system.booking_sys.get_booking_statuses()]
