		...
		<div class="grid-two">
			<form id="login-form" class="stack">
				...
				<div>
					...
					<input id="login-pwd" name="pwd" type="pwd" required>
				</div>
			...
			<form id="register-form" class="stack">
				...
				<div>
					...
					<input id="register-pwd" name="pwd" type="pwd" required>
				</div>
				<div>
					<label for="register-payment-pin">Payment PIN (4 digits)</label>
					<input id="register-payment-pin" name="payment_pin" type="pwd" inputmode="numeric" maxlength="4" required>
				</div>
...
async function handleLogin(event) {
	...
	const payload = {
		...
		pwd: String(formData.get("pwd") || "").trim(),
	};
...
async function createBooking(eventId, ticketTypeId, quantity, checkoutResultId = "checkout-result") {
	...
	try {
		...
		checkoutResult.innerHTML = `
			<div class="card">
				...
				<div>
					...
					<input id="checkout-pin-${booking.id}" type="pwd" inputmode="numeric" maxlength="4" placeholder="4-digit PIN" required>
				</div>
		...
		startCountdown(booking.id, booking.expire_time, booking.server_now, countdownNode, () => {
		...
		contentPanel.innerHTML = `
			...
			<form id="balance-form" class="stack">
				...
				<div>
					...
					<input id="add-balance-pin" type="pwd" inputmode="numeric" maxlength="4" placeholder="4-digit PIN" required>
				</div>
...
function renderTicketsPage(requestedPage = 1) {
			...
			(row) => `
				<article class="card" id="booking-${row.id}">
					...
					<div class="inline-actions">
						...
						${row.status === "pending" ? `<input type="pwd" id="pin-${row.id}" inputmode="numeric" maxlength="4" placeholder="Payment PIN">` : ""}
	...
	pageRows.filter((row) => row.status === "pending").forEach((row) => {
		...
		startCountdown(row.id, row.expire_time, row.server_now, countdownNode, () => {
			if (payButton) {
				payButton.disabled = true;
				payButton.textContent = "Expired";
				payButton.classList.add("warn");
	...
	try {
		...
		container.innerHTML = `
			<div class="card" style="margin-top: 0.5rem;">
				...
				${row.status === "pending" ? `<p>Expires at: ${row.expire_time}</p>` : ""}
			</div>
		`;
